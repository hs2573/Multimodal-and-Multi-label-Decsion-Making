...
data_location = ".\results\spectrograms_example\img_channel_10s\log\stage"
...
def train_CNN_stage(data_location,ch_typ,img_width=32, img_height=32):
    ...
    print("Testing R = ", f1)
    batch_size = 10
    nb_train_samples = (a+b+c+d+e+f)/batch_size
    nb_validation_samples = (a1+b1+c1+d1+e1+f1)/batch_size 
    epochs = 100
    ...
    checkpoint = ModelCheckpoint("first_level_model\vgg16_{}_stage.h5".format(ch_typ), monitor='val_accuracy', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
    ...
    hi_df.to_csv("first_level_model\hi_df_{}_stage.csv".format(ch_typ))
    
    # Save model
    custom_model.save('first_level_model\my_full_model_log_{}_stage.h5'.format(ch_typ))
    custom_model.save_weights('first_level_model\my_model_weights_log_{}_stage.h5'.format(ch_typ), overwrite=True)
    json_string = custom_model.to_json()
    with open('first_level_model\only_model_{}_stage.json'.format(ch_typ), 'w') as f:
          f.write(json_string)
    ...
    df_test.to_csv("probability vector\{}_val_data_stage.csv".format(ch_typ))
    lf_test = pd.DataFrame(validation_generator.classes)
    lf_test.to_csv("probability vector\{}_val_label_stage.csv".format(ch_typ))
    y_pred = np.argmax(Y_pred, axis=1)

    Test_pred = custom_model.predict_generator(train_generator,steps = nb_train_samples)
    df_train = pd.DataFrame(Test_pred)
    df_train.to_csv("probability vector\{}_train_data_stage.csv".format(ch_typ))
    lf_train = pd.DataFrame(train_generator.classes)
    lf_train.to_csv("probability vector\{}_train_label_stage.csv".format(ch_typ))
    ...
    df_ytest.to_csv("probability vector\{}_test_data_stage.csv".format(ch_typ))
    lf_ytest = pd.DataFrame(test_generator.classes)
    lf_ytest.to_csv("probability vector\{}_test_label_stage.csv".format(ch_typ))
...
def main():
    ...
    for ch_typ in MODALITIES:
        ...
        train_CNN_stage(data_location,ch_typ,32,32)
